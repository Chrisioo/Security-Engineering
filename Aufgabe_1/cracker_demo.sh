...
while read -r word; do                                                          # Lese jedes Wort aus der Datei words.txt                       
    for username in "${!USERS[@]}"; do                                          # Iteriere über alle User              
        ...
        if [[ "$generated_hash" == *"$hash"* ]]; then                    # Vergleiche den generierten Hash mit dem Zielhash
...
for username in "${!GEFUNDENE_PASSWOERTER[@]}"; do                              # Durchlaufen von Array GEFUNDENE_PASSWOERTER         
    echo "Password für Nutzer $username: ${GEFUNDENE_PASSWOERTER[$username]}"   # Ausgabe der gefundenen Passwörter mit Nutzernamen
done

# Ausgabe der Nutzer, für die keine Passwörter gefunden wurden
for username in "${!USERS[@]}"; do                                              # Durchlaufen von Array USERS
    echo "Passwort für Nutzer $username wurde nicht gefunden."                  # Ausgabe übriger Nutzer (wird entfernt, wenn Passwort gefunden)
done
